...
const resultAnalytics = document.getElementById("resultAnalytics");
...
async function renderResult(result) {
  ...
  resultAnalytics.textContent = "Loading...";
  try {
    const records = await fetchRecordsFromServer();
    const analytics = calculateMajorAnalytics(records);
    resultAnalytics.textContent = `${analytics.percentageFor(result.topMajor)}% of students also got: ${topInfo.title}`;
  } catch (err) {
    console.error("Could not load result analytics:", err);
    resultAnalytics.textContent = "Unavailable";
  }
